    ...
    <body>
        <main>
            ...
            if($_POST['mc']=="選擇店家"){
                ...
                if($a1){
                    echo "<header><h1>所有訂單</h1></header><form name='form' method='post' action='gaiorshan2.php'><select name='a'>";
                    while($a1){
                    ...
                    echo "<header><h1>全部暫無訂單</h1></header>";
                }
            }else{
                $a = mysqli_query ( $con,"select * from 訂單 where 店家名稱='{$_POST['mc']}' and 訂單備貨狀態=0;");
                $a1=mysqli_fetch_array($a);
                if($a1){
                    echo "<header><h1>",$_POST['mc'],"訂單</h1></header><form name='form' method='post' action='gaiorshan2.php'><select name='a'>";
                    while($a1){
                        echo "<option value='",$a1['訂單編號'],"'>",$a1['訂單編號'],"</option>";
                        $a1=mysqli_fetch_array($a);
                    }
        ...
        <form name="form2" method="post" action="gaiorshan0.php">
