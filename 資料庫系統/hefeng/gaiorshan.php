...
<html> 
    ...
    <body>
        <main>
            ...
            $a = mysqli_query ( $con,"select * from 訂單 where 店家名稱='{$_SESSION['mc']}' and 訂單備貨狀態=0;");
            $a1=mysqli_fetch_array($a);
            if($a1){
                echo "<header><h1>",$_SESSION['mc'],"訂單</h1></header><form name='form' method='post' action='gaiorshan_2.php'><select name='a'>";
                while($a1){
                ...
                echo "<header><h1>",$_SESSION['mc'],"暫無訂單</h1></header>";
            }
        ?>
        </form>
