...
void area(point* p, int index7, v* vt, int index4, bv* bvt, int index6);
void freeb(bline* b);
int main(void) {
	FILE* map;
	char s[10];
	...
	area(p2, index7, vt, index4, bvt, index6);
	freeb(&beachline);
	END = clock();
...
void area(point* p, int index7, v* vt, int index4, bv* bvt, int index6) {
	point* pv = new point[index4 + index6];
	int index = 0;
	point* px0y0 = new point[index7];
	int index2 = 0;
	point* px0ym = new point[index7];
	int index3 = 0;
	point* pxmy0 = new point[index7];
	int index5 = 0;
	point* pxmym = new point[index7];
	int index8 = 0;
	double min = maxx * maxx + maxy * maxy;
	double* ar = new double[index7];
	int index9 = 0;
	...
	for (int i = 0; i < index7; i++) {
		if (p[i].x * p[i].x + pow(p[i].y - maxy, 2) <= min) {
			min = p[i].x * p[i].x + pow(p[i].y - maxy, 2);
		}
	}
	for (int i = 0; i < index7; i++) {
		if (p[i].x * p[i].x + pow(p[i].y - maxy, 2) == min) {
	...
	min = maxx * maxx + maxy * maxy;
	for (int i = 0; i < index7; i++) {
		if (p[i].y * p[i].y + pow(p[i].x - maxx, 2) <= min) {
			min = p[i].y * p[i].y + pow(p[i].x - maxx, 2);
		}
	}
	for (int i = 0; i < index7; i++) {
		if (p[i].y * p[i].y + pow(p[i].x - maxx, 2) == min) {
	...
	min = maxx * maxx + maxy * maxy;
	for (int i = 0; i < index7; i++) {
		if (pow(p[i].x - maxx, 2) + pow(p[i].y - maxy, 2) <= min) {
	...
	for (int i = 0; i < index7; i++) {
		for (int j = 0; j < index4; j++) {
			if ((vt[j].p1.x == p[i].x && vt[j].p1.y == p[i].y) || (vt[j].p2.x == p[i].x && vt[j].p2.y == p[i].y) || (vt[j].p3.x == p[i].x && vt[j].p3.y == p[i].y)) {
				pv[index++] = vt[j].p;
			}
		}
		for (int j = 0; j < index6; j++) {
			if ((bvt[j].p1.x == p[i].x && bvt[j].p1.y == p[i].y) || (bvt[j].p2.x == p[i].x && bvt[j].p2.y == p[i].y)) {
				pv[index++] = bvt[j].p;
			}
		}
		for (int j = 0; j < index2; j++) {
			if (px0y0[j].x == p[i].x && px0y0[j].y == p[i].y) {
				pv[index].x = 0;
				pv[index++].y = 0;
				break;
			}
		...
		for (int j = 0; j < index3; j++) {
			if (px0ym[j].x == p[i].x && px0ym[j].y == p[i].y) {
				pv[index].x = 0;
				pv[index++].y = maxy;
				break;
			}
		...
		for (int j = 0; j < index5; j++) {
			if (pxmy0[j].x == p[i].x && pxmy0[j].y == p[i].y) {
				pv[index].x = maxx;
				pv[index++].y = 0;
				break;
			}
		...
		for (int j = 0; j < index8; j++) {
			if (pxmym[j].x == p[i].x && pxmym[j].y == p[i].y) {
				pv[index].x = maxx;
				pv[index++].y = maxy;
				break;
			}
		...
		for (int j = 0; j < index - 1; j++) {
			for (int k = 0; k < index - j - 1; k++) {
				if ((pv[k].x - center.x) * (pv[k + 1].y - center.y) - (pv[k + 1].x - center.x) * (pv[k].y - center.y) < 0) {
					point temp;
					temp = pv[k];
					pv[k] = pv[k + 1];
					pv[k + 1] = temp;
		...
		double a = 0;
		for (int j = 0; j < index; j++) {
			a += pv[j % index].x * pv[(j + 1) % index].y - pv[(j + 1) % index].x * pv[j % index].y;
		}
		a /= 2;
		ar[index9++] = a;
		index = 0;
	}
	for (int i = 0; i < index9 - 1; i++) {
		for (int j = 0; j < index9 - i - 1; j++) {
			if (ar[j + 1] < ar[j]) {
				double temp = ar[j];
				point t = p[j];
				p[j] = p[j + 1];
