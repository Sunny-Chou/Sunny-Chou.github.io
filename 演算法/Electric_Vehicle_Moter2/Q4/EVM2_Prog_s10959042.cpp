...
void print(bv* bvt, int index6 , v*, int);
...
int main(void) {
	...
	point* p = new point[index], * p2 = new point[index + 1];
	index = 0;
	int index7 = 0;
	fseek(map, 0, SEEK_SET);
	double START, END;
	...
	point* p3 = new point[index7 + 1];
	int index8 = 0;
	point op = dt(vt, &index4, bvt, &index6, p2, index7, p3, &index8);
	printf("新增點(%.2f,%.2f)\n", op.x, op.y);
	insertv(op, p2, &index7);
	for (int i = 0; i < index4; i++) {
		if (pow(vt[i].p.x - vt[i].p1.x, 2) + pow(vt[i].p.y - vt[i].p1.y, 2) > pow(op.x - vt[i].p.x, 2) + pow(op.y - vt[i].p.y, 2) || pow(vt[i].p.x - vt[i].p2.x, 2) + pow(vt[i].p.y - vt[i].p2.y, 2) > pow(op.x - vt[i].p.x, 2) + pow(op.y - vt[i].p.y, 2) || pow(vt[i].p.x - vt[i].p3.x, 2) + pow(vt[i].p.y - vt[i].p3.y, 2) > pow(op.x - vt[i].p.x, 2) + pow(op.y - vt[i].p.y, 2)) {
			insertv(vt[i].p1, p3, &index8);
			insertv(vt[i].p2, p3, &index8);
			insertv(vt[i].p3, p3, &index8);
			dv(vt, i, &index4);
			i--;
		}
	}
	for (int i = 0; i < index6; i++) {
		if (pow(bvt[i].p.x - bvt[i].p1.x, 2) + pow(bvt[i].p.y - bvt[i].p1.y, 2) > pow(op.x - bvt[i].p.x, 2) + pow(op.y - bvt[i].p.y, 2) || pow(bvt[i].p.x - bvt[i].p2.x, 2) + pow(bvt[i].p.y - bvt[i].p2.y, 2) > pow(op.x - bvt[i].p.x, 2) + pow(op.y - bvt[i].p.y, 2)) {
			insertv(bvt[i].p1, p3, &index8);
			insertv(bvt[i].p2, p3, &index8);
			dbv(bvt, i, &index6);
			i--;
		}
	}
	...
	int index10 = index4;
	while (index8 != 0) {
		if (index3 != 0 && etop(ev)->y <= ptop(p3).y && etop(ev)->y <= betop(bev)->y) {
			process_event(ev, &index3, vt, &index4, bev, &index5, bvt, &index6);
		}
		else if (index5 == 0 || ptop(p3).y <= betop(bev)->y) {
			process_point(p3, &index8, &b2, ev, &index3, bev, &index5);
		}
		else if (index5 != 0) {
			process_bevent(bev, &index5, bvt, &index6);
		}
	...
	checkdbv(p2, bvt, index9, &index6, index7);
	checkdv(p2, vt, index10, &index4, index7);
	freeb(&b2);
	delete[]p3;
	printf("充電站位置：\n");
